...
async function run() {
  ...
  // 1. Remove all MARKET SENTIMENT text
  content = content.replace(/\\n\\n📈 MARKET SENTIMENT:[^"\\]+/g, '');

  // 2. Replace makeStock function with one that formats the numbers nicely
  const oldMakeStockRegex = /export function makeStock\([\s\S]*?return \{[\s\S]*?\};\n\}/m;
  ...
  console.log('Cleaned up market sentiments and formatted numbers.');
}
